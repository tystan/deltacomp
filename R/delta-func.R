...
function(
  ...
  for(j in 1:n_covar){ #j<-1
    ...
    if(is.factor(this_covar)){
      ...
      m_cov[[j]]=this_covar[as.integer(this_covar)==m_temp][1]
...
# can also do ilrBase(D=n_comp) etc
  ...
  psi <- gsi.buildilrBase(sbp) ## The orthonormal transformation matrix


  ilr_comps <- ilr(dataf[,comps], V=psi)
...
################################# all predict changes  #################################
  ...
  cat("Comparison type being used: 'one-v-all'\n")
  ...
  m_delta
  ilr_means<-ilr(m_comps,V=psi)[1:n_preds,1:(n_comp-1)] # get rid of "rmult" class
  ilr_delta<-ilr(m_delta,V=psi)[1:n_preds,1:(n_comp-1)] # get rid of "rmult" class
  class(ilr_delta)
